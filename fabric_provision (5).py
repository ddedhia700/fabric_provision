...
def read_config(config_path: str, input_path: Optional[str] = None, user_inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Read fabric.yml; error if missing
    fabric_config = _read_yaml(config_path, "fabric configuration")
    if fabric_config:
        config.update(fabric_config)
    
    # Read input.yml if provided
    if input_path:
...
def _validate_config(config: Dict[str, Any]):
    ...
    # Validate network addresses if both provided
    if "transit" in config and "loopback" in config:
        try:
    ...
    # Validate spine port ranges if provided
    if "spine_port_channel_range" in config:
        ...
        for port in config["spine_port_channel_range"]:
...
def parse_flexible_range(range_input: Any) -> List[int]:
    ...
    if isinstance(range_input, str):
        ...
        range_str = ",".join(str(item) for item in range_input)
...
def assign_ips(devices: Dict[str, List[Dict]], num_leafs: int, config: Dict[str, Any]) -> None:
    ...
    loopback_ips = generate_ips(loopback_network, 1, 255)
    ...
    for leaf_idx, leaf in enumerate(devices["leafs"]):
        ...
        leaf["bgp_neighbors"] = [
            {"hostname": devices["spines"][0]["hostname"], "neighbor_ip": devices["spines"][0]["loopback_ip"], "remote_asn": config.get("bgp_asn", 65000)},
            {"hostname": devices["spines"][1]["hostname"], "neighbor_ip": devices["spines"][1]["loopback_ip"], "remote_asn": config.get("bgp_asn", 65000)}
        ]
        transit_index += 1
...
def get_user_input(input_path: Optional[str] = None) -> tuple[Dict[str, Any], List[str], List[str]]:
    ...
    if input_path:
        try:
            ...
            required_inputs = ["num_of_spines", "num_of_leafs", "spine_hostnames", "leaf_hostnames"]
    ...
    # Interactive input
    config = read_config("config/fabric.yml")
...
def main():
    ...
    try:
        ...
        generate_host_vars(devices, spines, args.dry_run)
