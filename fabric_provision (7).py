...
def read_config(config_path: str, input_path: Optional[str] = None, user_inputs: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    vrf_loopbacks = {}
    if "vrf" in config:
        for vrf_entry in config["vrf"]:
            for vrf_name, vrf_data in vrf_entry.items():
                if "loopbacks" in vrf_data:
                    for loopback_num in vrf_data["loopbacks"]:
                        vrf_loopbacks[loopback_num] = vrf_name
    config["vrf_loopbacks"] = vrf_loopbacks
...
def _validate_config(config: Dict[str, Any]):
    ...
    # Validate loopbacks if provided
    if "loopbacks" in config:
        if not isinstance(config["loopbacks"], list) or not all(isinstance(n, int) and n >= 0 for n in config["loopbacks"]):
            raise ValueError("loopbacks must be a list of non-negative integers")
    
    # Validate IP address availability for loopbacks
    if "num_of_spines" in config and "num_of_leafs" in config and loopback_net and "loopbacks" in config:
        total_loopbacks = (config["num_of_spines"] + config["num_of_leafs"]) * len(config["loopbacks"])
        ...
        if usable_ips < total_loopbacks:
            raise ValueError(f"Loopback network {loopback_net} has only {usable_ips} usable IPs, need {total_loopbacks} for {total_loopbacks} loopback interfaces")
    
    # Validate transit IP availability
    ...
    if "leaf_spine_ports" in config:
        try:
            ...
            if config.get("leaf_spine_link_agg") and len(config["leaf_spine_ports"]) != 2:
    ...
    if "inter_leaf_ports" in config:
        try:
            ...
            if config.get("inter_leaf_link_agg") and len(config["inter_leaf_ports"]) != 2:
...
def assign_ips(devices: Dict[str, List[Dict]], num_leafs: int, config: Dict[str, Any]) -> None:
    ...
    loopback_nums = config.get("loopbacks", [])
    vrf_loopbacks = config.get("vrf_loopbacks", {})
    
    transit_subnets = generate_subnets(transit_network, leaf_spine_linknet)
    
    # Assign loopback IPs for all devices
    total_loopbacks = len(devices["spines"]) * len(loopback_nums) + len(devices["leafs"]) * len(loopback_nums)
    loopback_ips = generate_ips(loopback_network, 1, total_loopbacks)
    
    ip_index = 0
    for device in devices["spines"] + devices["leafs"]:
        device["loopbacks"] = []
        for loopback_num in loopback_nums:
            loopback_ip = f"{loopback_ips[ip_index]}/32"
            loopback_entry = {
                "intf": f"Loopback{loopback_num}",
                "ip_addr": loopback_ip,
                "vrf": vrf_loopbacks.get(loopback_num)
            }
            device["loopbacks"].append(loopback_entry)
            ip_index += 1
        # Set primary loopback_ip for remote_bgp_neighbor and inventory
        device["loopback_ip"] = device["loopbacks"][0]["ip_addr"] if device["loopbacks"] else None
...
def generate_host_vars(devices: Dict[str, List[Dict]], spines: List[str], fabric_context: str, dry_run: bool = False):
    """Generate host_vars YAML files for leafs using fabric_context Jinja2 template."""
    ...
    for leaf in devices["leafs"]:
        ...
        for intf in leaf["interfaces"]:
            if intf["peer"] in spines:
                spine = next(s for s in devices["spines"] if s["hostname"] == intf["peer"])
                interfaces.append({
                    ...
                    "remote_bgp_neighbor": spine["loopback_ip"].split("/")[0]
                })
...
def generate_ip_assignments_csv(devices: Dict[str, List[Dict]], dry_run: bool = False):
    ...
    headers = ["Device", "Role", "Loopback IP", "Interface", "Transit IP", "Remote Device", "Remote Transit IP", "Transit Subnet", "VRF"]
    table = [headers, ["-" * 20, "-" * 10, "-" * 15, "-" * 20, "-" * 15, "-" * 20, "-" * 15, "-" * 15, "-" * 10]]
    
    for device in devices["spines"] + devices["leafs"]:
        # Add rows for loopback interfaces
        ...
        # Add rows for transit interfaces
